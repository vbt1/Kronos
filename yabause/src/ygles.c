...
YglTextureManager * YglTMInit(unsigned int w, unsigned int h) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, tm->width, tm->height, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
...
void YglTmPush(YglTextureManager * tm){
  ...
  if (tm->texture != NULL ) {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, tm->width, tm->yMax, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, 0);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
    tm->texture = NULL;
  }
  YabThreadUnLock(tm->mtx);
...
int YglGenFrameBuffer() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, _Ygl->width, _Ygl->height, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
  ...
  if (_Ygl->vdp1_pbo[0] == 0) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, 512, 256, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
...
void YglSetClearColor(u8 r, u8 g, u8 b){
  _Ygl->clear[0] = r;
  _Ygl->clear[1] = g;
  _Ygl->clear[2] = b;
  _Ygl->clear[3] = 0;
...
static void releaseVDP1FB(int i) {
  if (_Ygl->vdp1FrameBuff[i] != 0) {
    if (_Ygl->vdp1fb_buf[i] != NULL) {
      ...
      glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 512, 256, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, 0);
      glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
...
void YglRender(Vdp2 *varVdp2Regs) {
   ...
   if ((varVdp2Regs->BKTAU & 0x8000) != 0) {
     ...
     glClearBufferuiv(GL_COLOR, 0, _Ygl->clear);
     glClearBufferfi(GL_DEPTH_STENCIL, 0, 0, 0);
   }
...
u32 * YglGetColorRamPointer() {
  ...
  if (_Ygl->cram_tex == 0) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, 2048, 1, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  ...
  if (_Ygl->cram_tex_buf == NULL) {
    ...
    glTexSubImage2D(GL_TEXTURE_2D,0,0,0,2048, 1,GL_RGBA_INTEGER, GL_UNSIGNED_BYTE,_Ygl->cram_tex_buf);
  }
...
u32 * YglGetLineColorPointer(){
  ...
  if (_Ygl->lincolor_tex == 0){
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, 512, 1, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
void YglSetLineColor(u32 * pbuf, int size){
  ...
  //if (_Ygl->lincolor_buf == pbuf) {
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, _Ygl->linecolor_pbo);
    glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, size, 1, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, 0);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
...
u32 * YglGetPerlineBuf(YglPerLineInfo * perline, int linecount, int depth ){
  ...
  if (perline->lincolor_tex == 0){
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8UI, 512, depth, 0, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
void YglSetPerlineBuf(YglPerLineInfo * perline, u32 * pbuf, int linecount, int depth){
  ...
  glBindBuffer(GL_PIXEL_UNPACK_BUFFER, perline->linecolor_pbo);
  glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, linecount, depth, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE, 0);
  glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
